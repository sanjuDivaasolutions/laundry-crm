<!--
  ...
  -  *
  -  *  Last modified: 22/01/25, 5:53 pm
    ...
    <card-container :hide-footer="true" :hide-header="true">
        ...
        <template #body>
            <FormFields
                ...
                defaults-route-after-event="inwardDefaultsLoaded"
            />
        </template>
    </card-container>
</template>
...
import { useModuleFormStore } from "@modules@/inwards/inwardsFormStore";
    ...
    switch (props.mode) {
        case "create":
            ApiService.post(moduleFormStore.route, formEntry)
                .then((response) => {
                    $toastSuccess("Inward created successfully");
                    redirectIndex();
                })
                .catch((error) => {
                    $catchResponse(error);
        ...
        case "edit":
            ApiService.put(
                ...
                .then((response) => {
                    $toastSuccess("Inward updated successfully");
                    redirectIndex();
                })
                .catch((error) => {
                    $catchResponse(error);
...
onMounted(async () => {
    ...
    emitter.on("invoice-items-update", (data) => {
        emitter.emit("refresh-options", { endpoint: "warehouse-shelves" });
    });
    emitter.on("onUpdateProducts", moduleFormStore.onUpdateProducts);
