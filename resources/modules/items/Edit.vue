    ...
    <div class="card">
        <div class="card-header border-0 pt-6">
            <div class="card-title">
                <h2>Edit Item</h2>
                <span class="text-muted ms-3" v-if="form.code">{{ form.code }}</span>
            </div>
...
const loadFormData = async () => {
    ...
    try {
        ...
        // Load item data with services
        const response = await ApiService.get(`items/${itemId.value}/edit`);
        ...
        if (item.service_prices && Array.isArray(item.service_prices)) {
            item.service_prices.forEach(sp => {
                servicePrices[sp.service_id] = sp.price;
                servicePriceEnabled[sp.service_id] = true;
            });
        }
    } catch (error) {
        console.error("Error loading form data:", error);
        $toastError("Failed to load item data");
...
const submitForm = async () => {
    ...
    try {
        ...
        await ApiService.put(`items/${itemId.value}`, payload);
        ...
        if (error.response?.status === 422) {
            ...
            $toastError(error.response?.data?.message || "Failed to update item");
        }
    } finally {
        loading.value = false;
    }
