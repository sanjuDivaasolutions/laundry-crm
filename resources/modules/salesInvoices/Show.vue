<!--
  - /*
  -  *  Copyright (c) 2024 Divaa Solutions. All rights reserved.
  -  *
  -  *  This software is the confidential and proprietary information of Divaa Solutions
  -  *  ("Confidential Information"). You shall not disclose such Confidential Information and
  -  *  shall use it only in accordance with the terms of the license agreement you entered into
  -  *  with Divaa Solutions.
  -  *
  -  *  Unauthorized copying of this file, via any medium is strictly prohibited.
  -  *  Proprietary and confidential.
  -  *
  -  *  Last modified: 12/12/24, 11:57 am
  -  *  Written by Chintan Bagdawala, 2024.
  -  */
  -->

<template>
    <div class="container">
        <div class="card mb-5 mb-xl-10">
            <div class="card-body pt-9 pb-0">
                <div class="d-flex flex-wrap flex-sm-nowrap mb-3">
                    <div class="flex-grow-1">
                        <div
                            class="d-flex justify-content-between align-items-start flex-wrap mb-2"
                        >
                            <div class="d-flex flex-column">
                                <div class="d-flex align-items-center mb-2">
                                    <a
                                        href="javascript:void(0);"
                                        class="text-gray-800 text-hover-primary fs-2 fw-bold me-1"
                                        >{{ entry.invoice_number }}</a
                                    >
                                    <a href="#" title="Blue tick">
                                        <span
                                            class="svg-icon svg-icon-1 svg-icon-primary"
                                        >
                                            <inline-svg
                                                src="/media/icons/duotune/general/gen026.svg"
                                            />
                                        </span>
                                    </a>
                                </div>

                                <div
                                    class="d-flex flex-wrap fw-semobold fs-6 mb-4 pe-2"
                                >
                                    <a
                                        href="#"
                                        class="d-flex align-items-center text-gray-400 text-hover-primary me-5 mb-2"
                                    >
                                        <span class="svg-icon svg-icon-4 me-1">
                                            <inline-svg
                                                src="/media/icons/duotune/communication/com006.svg"
                                            />
                                        </span>
                                        <span>{{ entry.date }}</span>
                                    </a>
                                </div>
                            </div>
                            <div class="d-flex my-4">
                                <RouterLink
                                    title="Back to Sales Invoices"
                                    :to="{ name: 'sales-invoices.index' }"
                                    class="btn btn-sm btn-primary me-3"
                                    ><FormIcon icon="feather:arrow-left"
                                /></RouterLink>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="d-flex overflow-auto h-55px">
                    <ul
                        class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bold flex-nowrap"
                    >
                        <li class="nav-item">
                            <router-link
                                :to="{
                                    name: 'sales-invoices.show',
                                    params: { id: id, tab: 'overview' },
                                }"
                                @click.prevent="switchTab('overview')"
                                class="nav-link text-active-primary me-6"
                                :class="tab === 'overview' ? 'active' : ''"
                            >
                                Overview
                            </router-link>
                        </li>
                        <li class="nav-item">
                            <router-link
                                :to="{
                                    name: 'sales-invoices.show',
                                    params: { id: id, tab: 'payments' },
                                }"
                                @click.prevent="switchTab('payments')"
                                class="nav-link text-active-primary me-6"
                                :class="tab === 'payments' ? 'active' : ''"
                            >
                                Payments
                            </router-link>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
        <Overview
            v-if="tab === 'overview'"
            :id="id"
            :entry="entry"
            :fields="showFields"
        />
        <Payments
            v-if="tab === 'payments'"
            :id="id"
            :entry="entry"
            type="si"
            :auto-open-payment="autoOpenPayment"
            @payment-modal-opened="handlePaymentModalOpened"
        />
    </div>
</template>

<script setup>
import { useModuleFormStore } from "@modules@/salesInvoices/salesInvoicesFormStore";
import Overview from "./ShowComponents/Overview.vue";
import { computed, onBeforeMount, ref, watch } from "vue";
import { useRoute, useRouter } from "vue-router";
import FormIcon from "@common@/components/form/FormIcon.vue";
import Payments from "@modules@/salesInvoices/Payments.vue";

const route = useRoute();
const router = useRouter();

const moduleFormStore = useModuleFormStore();
const entry = computed(() => moduleFormStore.entry);
const showFields = computed(() => moduleFormStore.showFields);

const tab = ref("overview");
const id = ref(null);
const autoOpenPayment = ref(false);

const switchTab = (tabName) => {
    tab.value = tabName;
};

const syncRouteState = () => {
    const qsTab = route.params.tab;
    if (qsTab) {
        tab.value = qsTab;
    }
    const shouldOpen = route.query.makePayment === "1";
    if (shouldOpen) {
        tab.value = "payments";
    }
    autoOpenPayment.value = shouldOpen;
};

const handlePaymentModalOpened = () => {
    autoOpenPayment.value = false;
    if (route.query.makePayment) {
        router.replace({
            name: "sales-invoices.show",
            params: { id: id.value, tab: "payments" },
            query: {},
        });
    }
};

onBeforeMount(() => {
    id.value = route.params.id;
    moduleFormStore.loadShowData(id.value);
    syncRouteState();
});

watch(
    () => [route.params.tab, route.query.makePayment],
    () => {
        syncRouteState();
    }
);
</script>

<style scoped></style>
