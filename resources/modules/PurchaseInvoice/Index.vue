          ...
          <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer">
            ...
            <tbody class="fw-semibold text-gray-600">
              ...
              <tr v-else-if="invoices.data.length === 0">
                <td colspan="11" class="text-center">No purchase invoices found</td>
              </tr>
              <tr v-for="invoice in invoices.data" :key="invoice.id">
                ...
                <td>
                  <div v-if="invoice.supplier">
                    <div class="text-gray-800 fw-bold">{{ invoice.supplier.name }}</div>
                    <div class="text-muted fw-semibold text-muted d-block fs-7">{{ invoice.supplier.code }}</div>
                  </div>
                  <span v-else>-</span>
                </td>
                <td>{{ invoice.warehouse?.full_name || '-' }}</td>
...
export default {
  ...
  setup() {
    ...
    const fetchInvoices = async () => {
      loading.value = true
      try {
        const response = await api.get('/api/purchase-invoices', { params: filters })
        ...
        Swal.fire({
          icon: 'error',
          title: 'Error',
          text: 'Failed to fetch purchase invoices'
        })
      } finally {
        loading.value = false
      }
    ...
    const fetchSuppliers = async () => {
      try {
        const response = await api.get('/api/purchase-invoices/suppliers')
    ...
    const fetchWarehouses = async () => {
      try {
        const response = await api.get('/api/purchase-invoices/warehouses')
    ...
    const deleteInvoice = async (invoice) => {
      ...
      if (result.isConfirmed) {
        try {
          await api.delete(`/api/purchase-invoices/${invoice.id}`)
          await fetchInvoices()
          Swal.fire({
            icon: 'success',
            title: 'Deleted!',
