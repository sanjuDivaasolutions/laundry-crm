    ...
    <div class="card">
        <div class="card-header border-0 pt-6">
            <div class="card-title">
                <h2>Edit Order</h2>
                <span class="text-muted ms-3" v-if="form.order_number">{{ form.order_number }}</span>
            </div>
            ...
            <form v-else @submit.prevent="submitForm">
                ...
                <div class="row mb-8">
                    ...
                    <div class="col-md-3 mb-5">
                        ...
                        <input v-model="form.order_date" type="date" class="form-control" :class="{ 'is-invalid': errors.order_date }" />
                        <div v-if="errors.order_date" class="invalid-feedback">{{ errors.order_date[0] }}</div>
                    </div>
                    <div class="col-md-3 mb-5">
                        <label class="form-label">Promised Date</label>
                        <input v-model="form.promised_date" type="date" class="form-control" />
                    </div>
                    <div class="col-md-3 mb-5">
                        <label class="form-label">Urgent</label>
                        <div class="form-check form-switch mt-3">
                            <input v-model="form.urgent" type="checkbox" class="form-check-input" id="urgent" />
                            <label class="form-check-label" for="urgent">{{ form.urgent ? 'Yes - Rush Order' : 'No' }}</label>
                        </div>
                    </div>
                    <div class="col-md-9 mb-5">
                        <label class="form-label">Notes</label>
                        <input v-model="form.notes" type="text" class="form-control" placeholder="Special instructions..." />
                    </div>
                </div>
...
const loadFormData = async () => {
    ...
    try {
        ...
        const [orderRes, customersRes, itemsRes, servicesRes] = await Promise.all([
            ApiService.get(`orders/${orderId.value}/edit`),
        ...
        form.promised_date = order.promised_date || "";
        form.urgent = order.urgent;
...
const submitForm = async () => {
    ...
    try {
        await ApiService.put(`orders/${orderId.value}`, { ...form });
