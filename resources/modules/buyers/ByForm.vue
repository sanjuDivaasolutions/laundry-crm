...
import { useModuleFormStore } from "@modules@/buyers/buyersFormStore";
...
import { defaultEntry } from "./buyersFormData";
    ...
    (agentId) => {
        ...
        if (agentId) {
            ...
            entry.value.agent_name =
                agentData.display_name ?? agentData.name ?? entry.value.agent_name;
            return;
        }
    ...
    switch (props.mode) {
        case "create":
            ApiService.post(moduleFormStore.route, formEntry)
                .then((response) => {
                    $toastSuccess("Buyer Contact Information created successfully");
                    redirectIndex();
                })
                .catch((error) => {
                    $catchResponse(error);
        ...
        case "edit":
            ApiService.put(
                ...
                .then((response) => {
                    $toastSuccess("Buyer Contact Information updated successfully");
                    redirectIndex();
                })
                .catch((error) => {
                    $catchResponse(error);
