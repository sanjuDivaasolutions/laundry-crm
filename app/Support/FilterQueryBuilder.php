            ...
            $query->where(function ($query) use ($strFilter) {
                foreach ($strFilter as $item) {
                    if ($this->isNestedColumn($item['column'])) {
                        ...
                        /*$query->orWhereHas(Str::camel($callable), function ($q) use ($item) {
                            $this->{Str::camel($item['operator'])}(
                                $item,
                                $q
                            );
    ...
    /*protected function makeOrder($query, $data)
    {
        foreach ($data['order_column'] as $str) {
            if ($this->isNestedColumn($str)) {
                [$relationship, $column] = explode('.', $str);
        ...
        foreach ($data['order_column'] as $str) {
            if ($this->isNestedColumn($str)) {
                ...
                foreach ($relationships as $relationship) {
                    ...
                    $relatedModel = $belongsTo->getModel();
                    $relatedTable = $relatedModel->getTable();
                    $as = "{$prefix}{$relatedTable}";

                    $query->leftJoin(
                        "{$relatedTable} as {$as}",
                        "{$as}.id",
                        '=',
                        "{$currentModel->getTable()}.{$relationship}_id"
                    );

                    $currentModel = $relatedModel;
                    $prefix .= "{$relationship}_";
                }
